...
//! `DocumentService`, `TagService`, `ExportService`, `RevisionRestoreService`), and the one
//! Server-Sent Events feed. `rekall-mcp` never sees any of it, which is what keeps the MCP write
//! surface narrow.
...
impl ApiState {
    pub fn new(services: Services, stream: Arc<StepEventStream>) -> Self {
