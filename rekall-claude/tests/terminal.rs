...
async fn a_terminal_starts_claude_in_the_project_folder_with_rk_typed_and_echoes_input() {
    ...
    let view = terminals.open(task.id, Some(step.id), true, Some("opus"), Some("bogus")).await.unwrap();
    ...
    let again = terminals.open(task.id, None, false, None, None).await.unwrap();
...
async fn closing_a_terminal_kills_it_announces_the_end_and_releases_the_task() {
    ...
    let view = terminals.open(task.id, None, false, None, None).await.unwrap();
    let recorder = Arc::new(Recorder::default());
...
async fn opens_past_the_cap_a_missing_folder_or_a_missing_cli_are_refused() {
    ...
    terminals.open(one.id, None, false, None, None).await.unwrap();
    let refused = terminals.open(two.id, None, false, None, None).await.unwrap_err();
    ...
    let refused = world.terminals(8).open(task.id, None, false, None, None).await.unwrap_err();
    ...
    let refused = world.terminals(8).open(two.id, None, false, None, None).await.unwrap_err();
    assert_eq!(
        refused.message(),
