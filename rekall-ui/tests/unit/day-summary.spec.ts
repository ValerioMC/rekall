...
function taskStub(id: TaskId, projectId: ProjectId): Task {
  return {
    id,
...
const TASKS: Task[] = [
  taskStub('t1' as TaskId, 'p1' as ProjectId),
  taskStub('t2' as TaskId, 'p1' as ProjectId)
]
...
describe('summarizeByDay', () => {
  it('buckets a closed session under the day it started on', () => {
    const byDay = summarizeByDay([entry({})], TASKS, NOW)
    const rows = byDay.get('2026-08-29')
    expect(rows).toHaveLength(1)
    expect(rows![0]).toMatchObject({ taskId: 't1', projectId: 'p1', totalSeconds: 3600, isRunning: false })
  })
  ...
  it('counts an open session up to now, and marks it running', () => {
    const byDay = summarizeByDay([entry({ stoppedAt: null })], TASKS, NOW)
  ...
  it('resolves projectId to null when the task is no longer known', () => {
    const byDay = summarizeByDay([entry({ taskId: 'ghost' as TaskId })], TASKS, NOW)
    expect(byDay.get('2026-08-29')![0]!.projectId).toBeNull()
  })
})
